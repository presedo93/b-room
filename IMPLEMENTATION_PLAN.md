...
- 0.2 Create `src/db.py` (engine + session factory) — Not Started
- 0.3 Create `src/app.py` (AppContext / factory for wiring repos, services) — Not Started

Tests:
- Unit test for `get_session()` using a transient sqlite:///:memory: database that can create tables.
...
Tasks:
- 1.1 Create `src/domain/models.py` (Instrument domain model) — Not Started
- 1.2 Create `src/persistence/models.py` (SQLModel table definitions) — Not Started
- 1.3 Implement `src/persistence/repositories.py` (InstrumentRepository) — Not Started
- 1.4 Add tests for repo behaviors (upsert, list, get) using sqlite:///:memory: — Not Started
...
Tasks:
- 2.1 Create `src/adapters/exchanges/base.py` — Not Started
- 2.2 Create `src/adapters/exchanges/bybit.py` (wrap existing logic) — Not Started
- 2.3 Implement `src/services/exchange_service.py` — Not Started
- 2.4 Add tests for adapters (httpx/respx) and the service — Not Started
...
Tasks:
- 3.1 Create `src/pages/registry.py` — Not Started
- 3.2 Create `src/pages/exchanges.py` that uses the ExchangeService — Not Started
- 3.3 Create `src/pages/backtester.py` and wire it into the registry — Not Started
- 3.4 Update `main.py` to use the page registry (non-destructive switch) — Not Started
...
Tasks:
- 4.1 Create `src/strategies/base.py` — Not Started
- 4.2 Implement an example strategy `src/strategies/simple_ma.py` — Not Started
- 4.3 Implement `src/services/strategy_service.py` to run strategies — Not Started
- 4.4 Integrate strategy selection and execution in `src/pages/backtester.py` — Not Started
- 4.5 Add unit tests for strategies using synthetic/historical data — Not Started
...
Tasks:
- 5.1 Add/extend tests across modules and ensure >80% coverage for core logic — Not Started
- 5.2 Update CI (GitHub Actions) to run pytest and linting — Not Started
- 5.3 Remove or archive old, replaced modules and update documentation — Not Started
